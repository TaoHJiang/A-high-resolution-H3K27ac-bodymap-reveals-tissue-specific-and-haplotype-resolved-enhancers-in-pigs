pdf("/home/pencode/2024H3K27ac/tissue/tissue_peak_number_ac.pdf",width=14,height=6)
ggplot(datt, aes(Tissue, Number, fill = Group)) +
...
pdf("/home/pencode/2024H3K27ac/tissue/tissue_peak_len_ac.pdf",width=14,height=6)
ggplot(datt, aes(Tissue, Percentage, fill = Group)) +
geom_bar(stat="identity", width = 0.7)+
scale_fill_manual(values = c('#B3DE69', '#FDB462')) +
theme(panel.background = element_rect(fill = NA, color = 'black'), 
    axis.line = element_line(colour = 'black'), axis.text.x = element_text(angle = 45, hjust = 1)) +
labs(title = NULL, x = NULL, y = 'Genome (%)')+
 theme(legend.text=element_text(colour="black", size=12),panel.border=element_blank(),
       legend.position=c(0.75,0.9),legend.title = element_blank(),
         panel.grid.major.x = element_blank(),
         panel.grid.minor.x = element_blank(),
