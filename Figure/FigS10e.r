library(ggplot2)
datt=read.table("tissue_AS_peak_number.txt",head=T)
pdf("tissue_AS_peak_number.pdf",width=14,height=6)
ggplot(datt, aes(tissue, Freq, fill = Var1)) +
geom_bar(stat="identity", width = 0.7)+
scale_fill_manual(values = c("#FF3334","#0099FF","#83ba9e")) +
theme(panel.background = element_rect(fill = NA, color = 'black'),  
    axis.line = element_line(colour = 'black'), axis.text.x = element_text(angle = 45, hjust = 1)) +
labs(title = NULL, x = NULL, y = 'Number of AS_peaks')+
